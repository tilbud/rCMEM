...
zStarExample <- zToZstar(z=targetElevation, meanSeaLevel=meanSeaLevel, meanHighWater=meanHighWater)

ggplot(data=data.frame(z=targetElevation, zStar = zStarExample), aes(x=z, y=zStar)) +
  geom_line() +
  geom_vline(data=(z=data.frame(z=c(meanHighWater, meanSeaLevel), 
...
zReconstructExample <- convertZStarToZ(zStar = zStarExample, meanSeaLevel = meanSeaLevel, meanHighWater = meanHighWater)
...
weightedMHW <- sum(meanHighHighWaterSpring*springTidesPerYear + meanHighHighWater*higherHighTidesPerYear + meanHighWater*highTidesPerYear) /
  sum(highTidesPerYear, higherHighTidesPerYear, springTidesPerYear)

deliveredSedimentExample <- mapply(deliveredSedimentSimple, z=targetElevation, meanSeaLevel=meanSeaLevel, meanHighWater=weightedMHW,
                                   settlingVelocity = settlingVelocity, suspendedSediment=suspendedSediment)
...
_Question? Is there a lower limit on sediment delivery to the marsh in MEM? Is it meanLowWater or the lower vegetation line?_
...
deliveredSedimentExample2 <- mapply(deliveredSediment3TidalCycle, z=targetElevation, suspendedSediment=suspendedSediment, meanSeaLevel=meanSeaLevel,
                                    meanHighWater=meanHighWater, meanHighHighWater=meanHighHighWater, meanHighHighWaterSpring=meanHighHighWaterSpring, settlingVelocity=settlingVelocity)
...
totalRange <- ggplot(data=sedimentComparisonPlot, aes(x=elevation, y=deliveredSediment)) +
  ...
  geom_vline(data=data.frame(elevation=c(meanSeaLevel, meanHighWater, meanHighHighWater, meanHighHighWaterSpring),
                             datum=c("meanSeaLevel", "meanHighWater", "meanHighHighWater", "meanHighHighWaterSpring")), aes(xintercept=elevation,
                                                                          lty=datum))

sedimentComparisonPlotZoomedIn <- filter(sedimentComparisonPlot, elevation>meanHighWater)

# Zoom In
zoomedIn <- ggplot(data=sedimentComparisonPlotZoomedIn, aes(x=elevation, y=deliveredSediment)) +
  geom_line(lwd=1.25, aes(color=moduleType), alpha=0.6) +
  ylab(expression(paste("Delivered Sediment (g cm"^"-2",")", sep=""))) +
  ggtitle("Zoomed in Above meanHighWater")  +
  geom_vline(data=data.frame(elevation=c(meanHighWater, meanHighHighWater, meanHighHighWaterSpring),
                             datum=c("meanHighWater", "meanHighHighWater", "meanHighHighWaterSpring")), 
...
exScenarioCurve <- buildScenarioCurve(startYear = 2015, meanSeaLevel = meanSeaLevel, relSeaLevelRiseInit=0.3, 
                                      relSeaLevelRiseTotal = 100, suspendedSediment=30)
head(exScenarioCurve)

exScenarioCurve <- buildHighTideScenario(scenarioCurve=exScenarioCurve, meanHighWaterDatum=meanHighWater, meanHighHighWaterDatum=meanHighHighWater, 
                                         meanHighHighWaterSpringDatum=meanHighHighWaterSpring, lunarNodalAmp = lunarNodalAmp)
head(exScenarioCurve)

ggplot(data=exScenarioCurve) + 
  geom_ribbon(aes(x=years, ymax=meanHighWater, ymin=meanSeaLevel-(meanHighWater-meanSeaLevel)), alpha=0.6) +
  geom_line(aes(x=years, y=meanSeaLevel), color="black") +
  geom_point(aes(x=years, y=meanSeaLevel), color="black", pch=16) +
...
highTideScenarios <- exScenarioCurve %>%
  ...
  gather(value="elevation", key="high tide", -years, -meanSeaLevel) 

ggplot(data=exScenarioCurve) + 
  geom_line(aes(x=years, y=meanSeaLevel), color="black") +
  geom_point(aes(x=years, y=meanSeaLevel), color="black", pch=16) +
  geom_ribbon(data = highTideScenarios, aes(x=years, ymax=elevation, ymin=meanSeaLevel-(elevation-meanSeaLevel), 
...
scenarioNetElevationChange <- scenario %>%
  select(years, meanSeaLevel, surfaceElevation) %>%
