...
exZVegMin <- -28 # cm relative to datum
exZVegMax <- 351
exZVegPeak <- 209
exRootToShoot <- 2
exBgTurnover <- 0.5
...
displayAGB <- data.frame(elevation=seq((zVegMin-abs(zVegMin*.1)), 
                                       (zVegMax+abs(zVegMax*.1)), 
                                       by = ((zVegMax-zVegMin)/100) )) %>%
  mutate(`no peak evl.`= predictedBiomass(z=elevation, bMax=2500, zVegMax=3, zVegMin=-1, zVegPeak=NA),
         `w/ peak elv.`= predictedBiomass(z=elevation, bMax=2500, zVegMax=3, zVegMin=-1, zVegPeak=2)) %>%
  pivot_longer(cols = c('no peak evl.', 'w/ peak elv.'), names_to='parabolaType', values_to = 'agb')
...
bgbFromAgb <- function(agb, rootToShoot, bgTurnover) {agb*rootToShoot} 
...
# Now we will build MHW lines based on MSL, long-term tidal range and lunar nodal amplitude
buildHighTideScenarios <- function(year, MHW0, MSL0, MSL, lunarNodalAmp=5) {
  
  MHW <- MSL + (MHW0-MSL0) + (lunarNodalAmp * (sin(2*pi*(year-1983)/18.61)))
  
  return(MHW)
}

scenarioCurve <- buildScenarioCurve(year0 = 2015, MSL = exMSL, rslr0=0.3, rslrTotal = 100, ssc=30)
head(scenarioCurve)

scenarioCurve$MHW <- buildHighTideScenarios(year = scenarioCurve$years, MSL= scenarioCurve$MSL, MSL0 = exMSL, MHW0 = exMHW, lunarNodalAmp = exLunarNodalAmp)

ggplot(data=scenarioCurve) + 
  geom_ribbon(aes(x=years, ymax=MHW, ymin=MSL-(MHW-MSL)), alpha=0.6) +
  geom_line(aes(x=years, y=MSL), color="darkblue") +
  geom_point(aes(x=years, y=MSL), color="darkblue", pch=16) +
