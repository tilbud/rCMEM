...
agbFromElv <- predictedBiomass(z=initElev, bMax = bMax, zVegMax = zVegMax,
...
displayAGB <- data.frame(elevation=targetElevation) %>%
  mutate(`no peak evl.`= predictedBiomass(z=targetElevation, bMax=bMax, zVegMax=zVegMax, zVegMin=zVegMin, zVegPeak=NA),
         `w/ peak elv.`= predictedBiomass(z=targetElevation, bMax=bMax, zVegMax=zVegMax, zVegMin=zVegMin, zVegPeak=zVegPeak)) %>%
...
avaliableSediment <- function(floodPct, ssc, settlingVelocity, k=1) {
  ...
  availableSSC <- ifelse(floodPct < 1/settlingVelocity, # if the sediment column IS NOT able to clear
                         ssc * floodPct/(1/settlingVelocity) * k, # available ssc is total possible caputre 
...
deliveredSediment3TidalCycle <- function(z, ssc, MSL, MHW, MHHW, MHHWS,
  ...
  tidalCycles <- tidalCycles %>%
    ...
    dplyr::mutate(nTides = ifelse(z>datumHigh, 0, nTides), # number of tides
           ...
           availableSSC = avaliableSediment(floodPct=floodPct, ssc=ssc, settlingVelocity=settlingVelocity, k=k), # Call available sediment function
...
buildScenarioCurve <- function(startYear, endYear=startYear+99, MSL, rslr0=0.3, rslrTotal=100, ssc
                               #, presenceAbsence, speciesCode=names(presenceAbsence)
                               ) {
...
exScenarioCurve <- buildScenarioCurve(startYear = 2015, MSL = MSL, rslr0=0.3, rslrTotal = 100, ssc=30)
head(exScenarioCurve)

exScenarioCurve <- buildHighTideScenario(scenarioCurve=exScenarioCurve, MHW0=MHW, MHHW0=MHHW, MHHWS0=MHHWS, 
                                         lunarNodalAmp = lunarNodalAmp)
head(exScenarioCurve)
...
runMemWithCohorts <- function(startYear, endYear=startYear+99, rslrT1, rslrTotal, initElv,
                              ...
                              coreYear, coreDepth, coreMaxs, coreMins,
                              ...) {
  ...
  scenario <- buildScenarioCurve(startYear = startYear, endYear=endYear,
                                      MSL=MSL, rslr0=rslr0, rslrTotal=rslrTotal, ssc=ssc)
  
  # add high tides
  scenario <- buildHighTideScenario(scenario, MHW0=MHW, MHHW0=MHHW, MHHWS0=MHHWS, lunarNodalAmp = lunarNodalAmp)
  ...
  initAgb <- predictedBiomass(z=initElvStar, bMax = bMax, zVegMax = zStarVegMax, zVegMin = zStarVegMin, zVegPeak = zStarVegPeak)
  ...
  initSediment <- deliverSedimentFlexibly(z=initElv, ssc=scenario$ssc[1], 
                                          MSL=scenario$MSL[1], MHW=scenario$MHW[1], MHHW = scenario$MHHW[1], 
                                          MHHWS = scenario$MHHWS[1], settlingVelocity=settlingVelocity)
  
  depositInitSediment <- function(...){return(initSediment)}
  
  # Run initial conditions to equilibrium
  cohorts <- runToEquilibrium(totalRootMass_per_area=initBgb, rootDepthMax=rootDepthMax, rootTurnover=rootTurnover,
  ...
  return(list(annualTimeSteps = scenario, cohorts = trackCohorts))
}
...
ggplot(data=scenario, aes(x=years, y=mineral)) +
  geom_line() +
  ggtitle("Mineral Deposition Over Time")

ggplot(data=scenario, aes(x=years, y=cumulativeTotalOm)) +
  geom_line() +
...
ggplot(data=scenario, aes(x=years, y=omFlux)) +
  geom_step() +
