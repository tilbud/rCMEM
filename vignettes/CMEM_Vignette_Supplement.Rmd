...
3. The top and bottom depths of the cohort layers are recalculated using after changes in mass of the cohorts are transformed into volumes via user-specified self-packing densities (`omPackingDensity`, `mineralPackingDensity`, `rootPackingDensity`) via [`calculateDepthOfNonRootVolume()`].
