...
ggplot(parametersToRun %>%
         gather(key='variable', value='value', age:root_mass, `Total OM [g]`:`SOM [g-OM cm-3]`)) +
...
constSealevel <- defaultConsts
constSealevel$sedimentInputType <- c('constant', 'relative', 'dynamic')[2]
constSealevel$modernAge <- max(startingProfile$age)
constSealevel$meanHighWaterElevation <- max(startingProfile$layer_bottom) + defaultParms$depthBelowMHW
...
constRaising$modernAge <- max(startingProfile$age)
constRaising$meanHighWaterElevation <- function(year){
  return(max(startingProfile$layer_bottom) + 0.03*year)
}
