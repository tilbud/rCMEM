...
#source('R/convertProfile_AgeToDepth.R')
...
runs.df <- plyr::ddply(parms.df, c('rootDepthMax', 'totalRootBiomass'), function(xx){
  ...
  ans <- runToEquilibrium(rootTurnover = defaultConsts$rootTurnover, 
                   ...
                   ssc = defaultConsts$ssc,
                   meanTidalHeight = defaultConsts$depthBelowMHW, 
                   massLiveRoots.fn = massLiveRoots,
                   totalRootMass_per_area = xx$totalRootBiomass, #parameter we are changing
...
temp.df <- ddply(defaultParms2.0, c('totalRootBiomass', 'rootDepthMax'), as.factor(runToEquilibrium(rootTurnover = defaultConsts2.0$rootTurnover, 
                               rootOmFrac = defaultConsts2.0$rootOmFrac,
                               omDecayRate = defaultConsts2.0$omDecayRate,
                               packing = defaultConsts2.0$packing,
                               
                               mineralInput_g_per_yr.fn = sedimentInputs,
                               ssc = defaultConsts2.0$ssc, 
                               meanTidalHeight = defaultConsts2.0$depthBelowMHW, 
                               
                               massLiveRoots.fn = massLiveRoots,
                               totalRootMass_per_area = defaultParms2.df$totalRootBiomass,
...
print(short.data <- convertProfile_AgeToDepth(example.Run2.0, layerTop = c(0,10), layerBottom = c(10, 100)))
