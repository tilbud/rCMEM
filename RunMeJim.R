...
library(tidyverse)
  ...
  parm <- setNames(as.list(testCases$parameters.df$Run6), testCases$parameters.df$Sheet)
...
for(ii in 1:1000){
  ...
  endingProfile_byDepth <- convertProfile_AgeToDepth(endingProfile_byAge, 
                             layerTop = c(0, testCases$Run6$`Core Section or Depth (cm)`[-nrow(testCases$Run6)]),
                             layerBottom = testCases$Run6$`Core Section or Depth (cm)`)
  ...
  compareReady <- endingProfile_byDepth %>%
    ...
    left_join(testCases$Run6 %>% select(`Core Section or Depth (cm)`:`Mineral (g/cm2)`),
